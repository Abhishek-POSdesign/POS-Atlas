...
**Last updated:** 2026-07-29 (**AI history/future blindness fixed** — the AI never saw sleep/workout/task history beyond "today," root cause was a dead-code builder nothing called; replaced with a range-aware, past+future `explain_history` Fact Package and real routing. Full-page Calendar UI is planned next, pending mockup approval — see "Atlas AI — history & future awareness" below and SESSION_LOG.md 2026-07-29 entry) -- **Live at:** [atlas.abhisheksikka.com](https://atlas.abhisheksikka.com) -- **Current cache version:** `atlas-offline-shell-v64` -- **Latest migration:** `016_ai_notebook.sql`
...
**What's next (not yet built):** a full-page **Calendar** tab (past + future, replacing the earlier "History Calendar" framing) reading from the same new `db.js` range methods -- month grid, day cells showing category indicators (past: sleep/workout/checklist/tasks/project logs/journal; future: scheduled tasks/reminders/project items), inline (non-modal) Day Detail panel. Abhishek asked for 3 named mockup options (spacious/balanced/dense) via Artifact, using only `tokens.css` values, before any `calendar.js`/CSS is written -- that mockup round is the explicit next step, not yet done as of this entry. See `SESSION_LOG.md` for the full approved plan.
...
- **Calendar page (2026-07-29, now in progress, no longer just a future idea)** -- Abhishek opened this phase: a full-page "Calendar" tab (Today/Projects/Calendar), past + future, reading from the new `db.js` date-range methods (see "Atlas AI — history & future awareness" above). AI routing + history data pipeline already shipped; the UI itself is pending a 3-option mockup approval round (spacious / balanced / dense layouts, Artifact-based, tokens-only) before any `calendar.js`/CSS is written. The Upcoming modal remains the interim stand-in until that ships.
