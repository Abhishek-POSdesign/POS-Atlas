...
**Last updated:** 2026-07-29 (**TTS now reads full replies hands-free** — client-side truncation removed from `_speak()`, server `MAX_CHARS` raised 1,000→3,000 with graceful sentence-boundary truncation instead of a hard reject, server reports truncation via `X-Voice-Truncated` header; `atlas-tts-proxy` redeployed as function version 4 — see SESSION_LOG.md 2026-07-29 entry) -- **Live at:** [atlas.abhisheksikka.com](https://atlas.abhisheksikka.com) -- **Current cache version:** `atlas-offline-shell-v63` -- **Latest migration:** `016_ai_notebook.sql`
