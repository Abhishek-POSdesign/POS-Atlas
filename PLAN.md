...
**Last updated:** 2026-07-27 (Phase 5 Health panel visual refinement, post-Comet review) · **Live at:** [atlas.abhisheksikka.com](https://atlas.abhisheksikka.com) · **Current cache version:** `atlas-offline-shell-v32` · **Latest migration:** `015_sleep_morning_note.sql`
...
- **Notes redesigned as three selector-style chips (2026-07-27, v3):** "Tonight's summary" (reuses the existing `sleepSummary` getter, unchanged) / "Morning reflection" / "Context" — each its own `.health-chip` on `--surface-2`, stacked with `.health-chip-stack`. All three always render once any sleep data exists for the day; an empty chip shows italic placeholder text (`.health-chip-value.inactive`) instead of disappearing. Supersedes the July-27-morning `.health-note` divider treatment, which itself had superseded the original `.sleep-card-note` colored-chip box — two Comet-review passes on the same day.
- **Inline 14-day trend, now a sparkline (2026-07-27, v3):** replaced the compact bar chart with a bottom-anchored SVG line + soft gradient fill + dashed goal reference line, computed by the new `sleepSparkline` getter (`js/pages/today.js`) from `sleepTrendDays`/`sleepGoalMinutes` — only real logged nights become points, a missing night is a gap in the line (never a fake flat value). Chart height 46px → 80px. Coloring goes through CSS classes (`.health-spark-line`, `.health-spark-stop-a/b`, all `currentColor` + `color: var(--accent-sage)` on the wrapping `.health-spark-svg`), not inline `var()` in SVG attributes.
- **"Attach screenshot (future AI)" placeholder (2026-07-27, v3):** shrunk from a full-width dashed bar to a small inline text+icon link (`.health-attach-link`) — still `disabled`, no upload logic, no Supabase field.
