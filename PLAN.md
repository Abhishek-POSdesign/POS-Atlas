...
**Last updated:** 2026-07-29 (**Calendar tab shipped** — full-page past+future Calendar (Today/Projects/Calendar), same `db.js` range methods the AI's `explain_history` reads, month grid + category filters + inline Day Detail with drill-downs into the real existing Task/Project/Notebook surfaces. See "Atlas Calendar" below and SESSION_LOG.md 2026-07-29 entries) -- **Live at:** [atlas.abhisheksikka.com](https://atlas.abhisheksikka.com) -- **Current cache version:** `atlas-offline-shell-v65` -- **Latest migration:** `016_ai_notebook.sql`
...
- **Drill-downs reuse existing surfaces, no page/modal duplicated for navigation targets:**
  ...
  - **Sleep / Workout / Checklist rows** -- confirmed via code that none of these have an existing arbitrary-date editor anywhere in the app (Today's Sleep/Workout modals and the Checklist Log popup are all hardcoded to `todayIsoDate()`/`this.today`, no date param). Rather than modify those to accept a foreign date, Calendar has its own small inline editor for each, right inside the Day Detail section (`editingSleep`/`editingWorkout`/`editingChecklistItemId`), using the exact same `DB.Sleep.save`/`DB.Workout.save`/`DB.Checklist.setStatus` calls Today already uses, just parameterized by whichever date is selected. Checklist statuses are the real 3 values (`done`/`skipped`/`holiday`, mirrored from `checklist.js`'s own `confirmLog`/`clearLog`) -- `'missed'` is never a settable status, only the derived count of applicable-but-unmarked items for that day.
