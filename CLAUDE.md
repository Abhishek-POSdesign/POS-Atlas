...
- **Health and Tasks & Reminders share one row**, flex-grow weighted ~40/60 (Health smaller, Tasks larger) via `.dd-flex-health`/`.dd-flex-tasks` inside `.dd-row-2col` — not a fixed grid, so a day with only one of the two present still fills the row. **Checklist, Projects & Work Logs, and Journal each stay their own full-width row** — Abhishek's own explicit split, "they need big space." Don't move Checklist back into the 2-column share.
