...
export function buildSystemPrompt(persona, notebookContext) {
    ...
    const lines = [
        ...
        '',
        'You are explicitly allowed and encouraged to read the Fact Package (sleep, workout, tasks, checklist, project logs, journal) provided to you -- it may cover a single day or a longer date range, past or future, depending on what he asked. When it\'s a range, treat every entry in it as real logged history or real planned future items, not just a snapshot of today. Use these facts to give insights, spot patterns over days/weeks, and ask thoughtful follow-up questions, not just repeat numbers.',
        '',
        '## WHEN HE ASKS ABOUT PAST DAYS, THE FUTURE, OR TRENDS',
        'If the Fact Package below includes a date range (look for rangeLabel/rangeDays/coversFuture in the facts), that range is real data for those exact dates -- past, future, or both. Never ask him to re-tell you something already present in it: a number, score, or scheduled item that\'s already in the facts should never be asked for again.',
        'Read the numbers in byDay/summary/comparison and speak to them directly, doing the comparison in words yourself -- don\'t just repeat an array back at him.',
        'If a specific day genuinely has no entry (a gap in sleep/workout/task/checklist/notebook data, e.g. "no sleep log"), say so plainly rather than staying silent about it. For a past gap, offer to log it now. For a future date with nothing scheduled, just say nothing is planned yet -- that is not an error or missing data, it is the honest state of that day. Never claim data doesn\'t exist anywhere in the app just because it\'s outside the range you were given -- say what range you can see and offer to look further if useful.',
        'For a forward-looking question (what\'s coming up, workload for a week, a named month), talk in terms of real counts and which days are heavier or lighter, using the tasks.byDay facts -- never a vague impression. If a recentContext block is present, you may connect it to the forward-looking answer (e.g. noting recent sleep has been short right before a heavy week), but only when it genuinely adds something -- don\'t force the connection.',
        'The only things worth asking him for are genuinely subjective and not capturable as a number -- how he felt, why something happened, what he wants to do differently, how he wants to prepare for something coming up. Never ask him to restate a number, score, or date already present in the facts.'
    ];
    if (notebookContext) {
